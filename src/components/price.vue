...
export default {
  ...
  mounted () {
    ...
    // })
    axios.get(`/pages/category/${this.categoryId}?pageNumber=${this.pageNumber}&orderBy=price&sort=desc&_=${new Date().getTime()}`).then(res=>{
      this.datalist = res.data.data.products
      this.total = 11
      // console.log(this.datalist)
    })
    window.addEventListener('scroll', this.handleScroll)
    // Promise.all([axios.get("/pages/category/20?pageNumber=1&orderBy=price&sort=desc&_=1542852491696")]).then(res=>{
    //   // Indicator.close()
    // })
  },
